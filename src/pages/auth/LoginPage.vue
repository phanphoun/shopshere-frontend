<template>
  <div>
    ...
    <div class="flex items-center justify-center gap-4">
      ...
      <a href="#" @click.prevent="googleLogin" class="w-11 h-11 rounded-full bg-white border border-gray-200 text-gray-700 flex items-center justify-center hover:bg-gray-50 transition-colors" aria-label="Login with Google">
...
async function googleLogin() {
  try {
    const url = await authStore.socialLogin('google')
    ...
    // Store the intended redirect URL in localStorage before opening the popup
    // so SocialCallbackPage can read it (no session in API routes to carry it).
    const redirectAfter = route.query.redirect || '/'
    localStorage.setItem('social_next', redirectAfter)

    // Listen for postMessage from the popup after social auth completes
    const authResult = await new Promise((resolve, reject) => {
      ...
      // Also handle the case where the popup is closed without completing auth
    ...
    await authStore.socialCallback({ token: authResult.token, user: authResult.user })
    ...
    if (e.message !== 'Google login was cancelled.') {
      toastStore.error(e.formattedMessage || e.message || 'Google login failed.')
    }
  }
}
